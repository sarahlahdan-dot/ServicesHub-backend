...
const addCartItem = async (req, res) => {
  try {
    ...
    if (!cart) cart = await Cart.create({ customerId: req.user.id, items: [] });
    ...
    await cart.save();
    const populated = await Cart.findOne({ customerId: req.user.id }).populate(
      "items.serviceId",
    );
...
module.exports = { getCart, addCartItem, removeCartItem, checkoutCart };
