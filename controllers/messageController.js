...
const ensureUserCanMessage = (currentUser, targetUserId) => {
  ...
  if (currentUserId === String(targetUserId)) {
    return {
      error: { status: 400, message: "You cannot send a message to yourself." },
    };
  }
...
const listUserMessages = async (req, res) => {
  try {
    ...
    if (targetUserResult.error) {
      return res
        .status(targetUserResult.error.status)
        .json({ message: targetUserResult.error.message });
    }
    ...
    if (permissionResult.error) {
      return res
        ...
        .json({ message: permissionResult.error.message });
    }
    ...
    const messages = await Message.find({
      $or: [
        ...
        },
        {
          ...
          receiverId: permissionResult.currentUserId,
        },
      ],
    })
...
module.exports = { listUserMessages, sendUserMessage };
